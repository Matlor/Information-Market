  ...
  const refreshStatusMap = (map) => {
    ...
    <div className="relative overflow-x-auto shadow-md sm:rounded-lg"> 
      ...
      <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        <thead className="text-xs text-gray-700 bg-gray-50 dark:bg-gray-200 dark:text-gray-400">
          <tr>
            ...
            <th scope="col" className="px-6 py-3">
              <button onClick={() => {setOrderField("reward"); setOrderIsAscending(!orderIsAscending);}}>Reward { getArrow("reward") }</button>
            </th>
            <th scope="col" className="px-6 py-3">
              <button onClick={() => {setOrderField("status_end_date"); setOrderIsAscending(!orderIsAscending);}}>Time left { getArrow("status_end_date") }</button>
            </th>
          </tr>
        </thead>
        <tbody>
      ...
      <span className="text-sm text-gray-700 dark:text-gray-400">
        Showing <span className="font-semibold text-gray-900 dark:text-white">{totalQuestions == 0 ? 0 : pageIndex * questionsPerPage + 1}</span> to <span className="font-semibold text-gray-900 dark:text-white">{Math.min((pageIndex + 1) * questionsPerPage, totalQuestions)}</span> of <span className="font-semibold text-gray-900 dark:text-white">{totalQuestions}</span> Entries
      </span>
          <div className="inline-flex mt-2 xs:mt-0">
          <button disabled={pageIndex < 1} onClick={() => {setPageIndex(pageIndex - 1)}} className="inline-flex items-center py-2 px-4 text-sm font-medium text-white bg-gray-800 rounded-l hover:bg-gray-900 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
