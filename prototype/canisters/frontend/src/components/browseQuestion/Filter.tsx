const Filter = () => {
	return (
		<div className="w-[163px] flex justify-between items-center gap-[79px] py-[10px] px-[15px]  shadow-md rounded-md bg-colorBackgroundComponents">
			<div className="heading3-18px">Filter</div>
			<svg
				width="14"
				height="9"
				viewBox="0 0 14 9"
				fill="none"
				xmlns="http://www.w3.org/2000/svg"
			>
				<path
					d="M0.101701 0.742969L6.63212 8.82646C6.81904 9.05785 7.17897 9.05785 7.36788 8.82646L13.8983 0.742969C14.1409 0.441536 13.9222 0 13.5304 0H0.469584C0.0778387 0 -0.140902 0.441536 0.101701 0.742969Z"
					fill="#969696"
				/>
			</svg>
		</div>
	);
};

export default Filter;
