...
async function createQuestion(author: String, invoice_id: String, creation_date: number, content: String, reward: number): Promise<String>{
  const result = await sudographActor.mutation(gql`
    mutation ($author: String!, $invoice_id: ID!, $creation_date: Int!, $content: String!, $reward: Int!) {
      createQuestion(
        input: {author: $author, author_invoice: {connect: $invoice_id}, creation_date: $creation_date, status: OPEN, status_update_date: $creation_date, content: $content, reward: $reward}
      ) { id }
    }`,{ author, invoice_id, creation_date, content, reward });
...
const loadScenario = async (names: Array<String>, questionNumber: number) => {
  ...
  // @{
  const now_minutes : number = Math.floor(Date.now() / 60000); // Date now is in milliseconds, convert to minutes
  ...
  for (let i = 0; i < numToCreate; i++){
    ...
    let question = await createQuestion(questionAuthor.name, await createInvoice(questionAuthor.identity), now_minutes, lorem.generateWords(), reward);
    ...
    while(users.length > 0)
    {
      ...
      {
        answerSet.add(await createAnswer(question, author.name, now_minutes, lorem.generateWords()));
  ...
  for (let [question, answers] of questionMap) {
    ...
    if (rand < 40){ // 40% of questions will be OPEN
      await setStatus(question, "OPEN", now_minutes);
    }
    else if (rand < 55){ // 15% of questions will be PICKANSWER
      await setStatus(question, "PICKANSWER", now_minutes);
    }
    else if (rand < 75){ // 20% (max) of questions will be DISPUTABLE
      if (answers.size == 0){
        await setStatus(question, "CLOSED", now_minutes);
      } else {
        await setWinner(question, [...answers][0]);
        await setStatus(question, "DISPUTABLE", now_minutes);
      }
    }
      ...
      if (answers.size == 0){
        ...
        await setStatus(question, "DISPUTED", now_minutes);
      }
    }
