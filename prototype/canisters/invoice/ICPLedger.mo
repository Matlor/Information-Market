...
module {
  ...
  public func transfer (args : TransferArgs, ledgerActor:LedgerTypes.Interface) : async TransferResult {
    let result = await ledgerActor.transfer(args);
    switch result {
      case (#Ok index) {
  ...
  public func balance(args : AccountArgs, ledgerActor:LedgerTypes.Interface) : async BalanceResult {
    switch (Hex.decode(args.account)) {
      ...
      case (#ok account) {
        let balance = await ledgerActor.account_balance({account = Blob.fromArray(account)});
        #ok({
  ...
  public func verifyInvoice(args : ICPVerifyInvoiceArgs, ledgerActor:LedgerTypes.Interface) : async T.VerifyInvoiceResult {
    let i = args.invoice;
    ...
    switch destinationResult {
      ...
      case (#ok destination) {
        let balanceResult = await balance({account = destination}, ledgerActor);
